// api/auth/forgot-password.js (from saas-starter-kit)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (user) {
      if (user.auth_provider === 'google' && !user.password_hash) {
        ...
        const frontendUrl = process.env.VERCEL_URL 
          ...
          : process.env.FRONTEND_URL || 'https://claudesim.app';
    ...
    res.json({ success: true, message: 'If an account with this email exists, you will receive a password reset link shortly.' });
  } catch (error) {
